...
fn is_armstrong_number(num: u32) -> bool {
  ...
  if digit_count > 8 {
      let res = digits.iter().map(|&digit| powm(digit, digit_count)).sum();

      num == res
  } else {
      let res = digits.iter().map(|&digit| pow(digit, digit_count)).sum();

      num == res
  }
}

fn pow(mut a: u32, mut b: u32) -> u32 {
  let mut res = 1;
...
fn powm(mut a: u32, mut b: u32) -> u32 {
  let m = 10;
  a %= m;
  let mut res = 1;
