...
fn luhn(code: &str) -> bool {
  let mut sum = 0;
  let mut double = false;
  let s = code.chars().rev().collect::<String>().replace(" ", "");

  if code.trim().is_empty() || code.trim().len() < 2 {
      false
  } else {
      for c in s.chars() {
          ...
          if double == true {
              let num = (c as usize - '0' as usize) * 2;
              if num > 9 {
                  sum += num - 9;
              } else {
                  sum += num;
...
fn process_valid_case(number: &str, is_luhn_expected: bool) {
  assert_eq!(luhn(number), is_luhn_expected);
}
#[test]
