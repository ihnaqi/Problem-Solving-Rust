pub fn main() {
    let array: [i32; 2] = [1, 2];
    let k = 2;
    let rs = find(&[1, 3, 5, 8, 13, 21, 34, 55, 89, 144, 233, 377, 634], 144);

    println!("The index of {k} is {:?} ", rs);
}

fn find(array: &[i32], k: i32) -> Option<usize> {
    if array.is_empty() || k < array[0] || k > array[array.len() - 1] {
        None
    } else {
        let mut low = 0;
